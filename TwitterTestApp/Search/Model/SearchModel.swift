...
class SearchModel: NSObject {
    ...
    func searchTweets(_ text: String, _ afterAction:@escaping (Array<TweetModel>) -> Void, _ errorAction:@escaping (String) -> Void) {
        ...
        if let session = sessionStore.session() {
            self.isLoading = true
            TwitterAPIUtil.requestSearch(session.userID, text, "", afterTask, errorTask)
            return
        }
    ...
    func moreSearch(_ text: String, _ maxId:String, _ afterAction:@escaping (Array<TweetModel>) -> Void, _ errorAction:@escaping (String) -> Void) {
        ...
        if let session = sessionStore.session() {
            self.isLoading = true
            TwitterAPIUtil.requestSearch(session.userID, text, maxId, afterTask, errorTask)
        }
        else {
